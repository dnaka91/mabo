...
pub(super) fn compile_struct(
    ...
    quote! {
        ...
        impl #generics ::mabo::Encode for #name #generics #generics_where {
            #[allow(
...
fn compile_variant(
    ...
    let id = quote! { ::mabo::VariantId::new(#id) };
    let name = Ident::new(name, Span::call_site());
    ...
    match fields.kind {
        FieldKind::Named => quote! {
            Self::#name{ #(#field_names,)* } => {
                ::mabo::buf::encode_variant_id(w, #id);
                #fields_body
            }
        },
    ...
    if fields.kind == FieldKind::Unit {
        ...
        let calls = fields.fields.iter().map(|Field { name, ty, id, .. }| {
            ...
            if let Type::Option(ty) = &ty {
                ...
                let id = quote! { ::mabo::FieldId::new(#id, #enc) };
                quote! { ::mabo::buf::encode_field_option(w, #id, #name, |w, v| { #ty; }); }
            } else {
                let (enc, ty) = compile_data_type(opts, ty, name.into_token_stream(), true);
                let id = quote! { ::mabo::FieldId::new(#id, #enc) };
                quote! { ::mabo::buf::encode_field(w, #id, |w| { #ty; }); }
            }
        });
...
fn compile_generics(types: &[&str]) -> (TokenStream, TokenStream) {
    (!types.is_empty())
        .then(|| {
            ...
            (
                quote! { <#(#types,)*> },
                quote! { where #(#types2: ::mabo::buf::Encode + ::mabo::buf::Size,)* },
            )
        })
        .unwrap_or_default()
}
...
fn compile_data_type(
    ...
    match &ty {
        Type::Bool => (
            ...
            quote! { ::mabo::buf::encode_bool(w, *#name) },
        ),
        Type::U8 => (
            quote! { ::mabo::FieldEncoding::Fixed1 },
            quote! { ::mabo::buf::encode_u8(w, *#name) },
        ),
        Type::U16 => (
            quote! { ::mabo::FieldEncoding::Varint },
            quote! { ::mabo::buf::encode_u16(w, *#name) },
        ),
        Type::U32 => (
            quote! { ::mabo::FieldEncoding::Varint },
            quote! { ::mabo::buf::encode_u32(w, *#name) },
        ),
        Type::U64 => (
            quote! { ::mabo::FieldEncoding::Varint },
            quote! { ::mabo::buf::encode_u64(w, *#name) },
        ),
        Type::U128 => (
            quote! { ::mabo::FieldEncoding::Varint },
        ...
        Type::I16 => (
            quote! { ::mabo::FieldEncoding::Varint },
            quote! { ::mabo::buf::encode_i16(w, *#name) },
        ),
        Type::I32 => (
            quote! { ::mabo::FieldEncoding::Varint },
            quote! { ::mabo::buf::encode_i32(w, *#name) },
        ),
        Type::I64 => (
            quote! { ::mabo::FieldEncoding::Varint },
            quote! { ::mabo::buf::encode_i64(w, *#name) },
        ),
        Type::I128 => (
            quote! { ::mabo::FieldEncoding::Varint },
        ...
        Type::F32 => (
            ...
            quote! { ::mabo::buf::encode_f32(w, *#name) },
        ),
        Type::F64 => (
            quote! { ::mabo::FieldEncoding::Fixed8 },
            quote! { ::mabo::buf::encode_f64(w, *#name) },
        ),
        Type::String | Type::StringRef | Type::BoxString => (
            quote! { ::mabo::FieldEncoding::LengthPrefixed },
            quote! { ::mabo::buf::encode_string(w, #name) },
        ),
        Type::Bytes | Type::BytesRef | Type::BoxBytes => match opts.bytes_type {
            BytesType::VecU8 => (
                quote! { ::mabo::FieldEncoding::LengthPrefixed },
                quote! { ::mabo::buf::encode_bytes_std(w, #name) },
            ),
            BytesType::Bytes => (
                quote! { ::mabo::FieldEncoding::LengthPrefixed },
                quote! { ::mabo::buf::encode_bytes_bytes(w, #name) },
            ),
        },
        Type::Vec(ty) => {
            ...
            (quote! { ::mabo::FieldEncoding::LengthPrefixed }, {
                quote! { ::mabo::buf::encode_vec(w, #name, |v| { #size }, |w, v| { #encode; }) }
            })
        }
        Type::HashMap(kv) => {
        ...
        Type::HashSet(ty) => {
            ...
            (
                quote! { ::mabo::FieldEncoding::LengthPrefixed },
                quote! { ::mabo::buf::encode_hash_set(w, #name, |v| { #size }, |w, v| { #encode; }) },
            )
        }
        Type::Option(ty) => {
            let (_, encode) = compile_data_type(opts, ty, quote! { v }, false);
            (
                quote! { ::mabo::FieldEncoding::LengthPrefixed },
                quote! { ::mabo::buf::encode_option(w, #name, |w, v| { #encode; }) },
            )
        }
        Type::NonZero(ty) => match &**ty {
            Type::U8 => (
                quote! { ::mabo::FieldEncoding::Fixed1 },
                quote! { ::mabo::buf::encode_u8(w, #name.get()) },
            ),
            Type::U16 => (
                quote! { ::mabo::FieldEncoding::Varint },
                quote! { ::mabo::buf::encode_u16(w, #name.get()) },
            ),
            Type::U32 => (
                quote! { ::mabo::FieldEncoding::Varint },
                quote! { ::mabo::buf::encode_u32(w, #name.get()) },
            ),
            Type::U64 => (
                quote! { ::mabo::FieldEncoding::Varint },
                quote! { ::mabo::buf::encode_u64(w, #name.get()) },
            ),
            Type::U128 => (
                quote! { ::mabo::FieldEncoding::Varint },
            ...
            Type::I16 => (
                quote! { ::mabo::FieldEncoding::Varint },
                quote! { ::mabo::buf::encode_i16(w, #name.get()) },
            ),
            Type::I32 => (
                quote! { ::mabo::FieldEncoding::Varint },
                quote! { ::mabo::buf::encode_i32(w, #name.get()) },
            ),
            Type::I64 => (
                quote! { ::mabo::FieldEncoding::Varint },
                quote! { ::mabo::buf::encode_i64(w, #name.get()) },
            ),
            Type::I128 => (
                quote! { ::mabo::FieldEncoding::Varint },
            ...
            2..=12 => {
                ...
                (
                    ...
                    if root {
                        ...
                        quote! { ::mabo::buf::encode_tuple(w, || { #(#size)+* }, |w| { #(#encode;)* }) }
                    } else {
                        quote! { #(#encode;)* }
                    },
                )
        ...
        Type::Array(ty, _size) => {
            ...
            (
                quote! { ::mabo::FieldEncoding::LengthPrefixed },
                quote! { ::mabo::buf::encode_array(w, #name, |v| { #size }, |w, v| { #encode; }) },
            )
        }
        Type::External(_) => (
            quote! { ::mabo::FieldEncoding::LengthPrefixed },
