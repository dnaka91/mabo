...
pub struct MaboProjectList(Vec<MaboProject>);

#[no_mangle]
pub extern "C" fn mabo_project_discover(
    path: &c_char,
    out: &mut *mut MaboProjectList,
) -> ErrorCode {
    ...
    let ptr = Box::leak(Box::new(MaboProjectList(
        project.into_iter().map(MaboProject).collect(),
    )));
...
#[no_mangle]
pub extern "C" fn mabo_project_list_free(ptr: NonNull<MaboProjectList>) {
    drop(unsafe { Box::from_raw(ptr.as_ptr()) });
}

#[no_mangle]
pub extern "C" fn mabo_project_list_len(list: &MaboProjectList) -> usize {
    list.0.len()
}

#[no_mangle]
pub extern "C" fn mabo_project_list_get(
    list: &MaboProjectList,
    index: usize,
) -> *const MaboProject {
    &list.0[index]
}

pub struct MaboProject(Project);

#[no_mangle]
pub extern "C" fn mabo_project_load(path: &c_char, out: &mut *mut MaboProject) -> ErrorCode {
    let path = unsafe { CStr::from_ptr(path) };
    let path = OsStr::from_bytes(path.to_bytes());
    ...
    let ptr = Box::leak(Box::new(MaboProject(project)));
...
#[no_mangle]
pub extern "C" fn mabo_project_free(ptr: NonNull<MaboProject>) {
    drop(unsafe { Box::from_raw(ptr.as_ptr()) });
}

#[derive(Debug)]
pub struct MaboSchema<'a>(mabo_parser::Schema<'a>);

#[no_mangle]
pub extern "C" fn mabo_schema_parse<'a>(
    input: &'a c_char,
    path: Option<&'a c_char>,
    out: &mut *mut MaboSchema<'a>,
) -> ErrorCode {
    ...
    let ptr = Box::leak(Box::new(MaboSchema(schema)));
...
#[no_mangle]
pub extern "C" fn mabo_schema_free(ptr: NonNull<MaboSchema<'_>>) {
    drop(unsafe { Box::from_raw(ptr.as_ptr()) });
}

#[no_mangle]
pub extern "C" fn mabo_schema_validate(schema: &MaboSchema<'_>) -> bool {
    mabo_compiler::validate_schema(&schema.0).is_ok()
}

pub struct MaboSimplifiedSchema<'a>(mabo_compiler::simplify::Schema<'a>);

#[no_mangle]
pub extern "C" fn mabo_schema_simplify<'a>(
    schema: &'a MaboSchema<'_>,
) -> *mut MaboSimplifiedSchema<'a> {
    let simplified = mabo_compiler::simplify_schema(&schema.0);

    Box::leak(Box::new(MaboSimplifiedSchema(simplified)))
}

#[no_mangle]
pub extern "C" fn mabo_simplified_schema_free(ptr: NonNull<MaboSimplifiedSchema<'_>>) {
    drop(unsafe { Box::from_raw(ptr.as_ptr()) });
}
...
#[no_mangle]
pub extern "C" fn mabo_simplified_schema_to_json(schema: &MaboSimplifiedSchema<'_>) -> *mut c_char {
...
mod tests {
    ...
    use crate::{ErrorCode, MaboSchema, MaboSimplifiedSchema};
    ...
    #[test]
    fn validate() {
        let schema = MaboSchema(
            mabo_parser::Schema::parse("struct Sample { value: u32 @1 }", None).unwrap(),
        );
    ...
    fn to_json() {
        ...
        let schema = MaboSimplifiedSchema(mabo_compiler::simplify_schema(&schema));
