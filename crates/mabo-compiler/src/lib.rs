//! Compiler for Mabo schema files.
//!
...
impl IdGenerator {
    ...
    fn next(&mut self, id: Option<&mabo_parser::Id>) -> u32 {
        let id = id.map_or(self.next_id, mabo_parser::Id::get);
        self.next_id = id + 1;
        id
    }

    /// Get the next ID, but additionally combine it with a span to construct a new
    /// [`mabo_parser::Id`] instance.
    ///
    ...
    fn next_with_span(
        &mut self,
        id: Option<&mabo_parser::Id>,
        fallback: impl FnOnce() -> mabo_parser::Span,
    ) -> mabo_parser::Id {
