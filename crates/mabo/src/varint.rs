...
/// Use the _ZigZag_ scheme to encode a `BigInt` as `BigUint`.
...
/// Convert a _ZigZag_ encoded `BigInt` back to its original data.
...
pub fn size_ibig(value: &BigInt) -> usize {
    max(1, (zigzag_encode_ibig(value).bits() as usize + 6) / 7)
}
