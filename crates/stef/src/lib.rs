//! Runtime support crate for the STEF encoding format.
...
/// Convenience macro to include generated Rust code for Stef schemas.
///
/// By default build scripts write output files into a special directory provided by Cargo as
/// `OUT_DIR` environment variable. The `stef-build` crate additional puts all generated files into
/// a `stef/` sub-folder instead of placing the files at the root.
///
...
/// # Example
///
/// Assuming a schema file named `sample.stef` that is being compiled the `stef-build` crate, the
/// final output destination would be:
///
/// ```txt
/// $OUT_DIR/stef/sample.rs
/// ```
#[macro_export]
macro_rules! include {
    ($name:literal) => {
...
impl<T> NonZero<T> {
    /// ```
    /// let value = stef::NonZero::<String>::new("hello".to_owned()).unwrap();
...
/// String (Stef's `non_zero<string>`) that is guaranteed to not be empty.
pub type NonZeroString = NonZero<String>;
/// Byte vector (Stef's `non_zero<bytes>`) that is guaranteed to not be empty.
pub type NonZeroBytes = NonZero<Vec<u8>>;
/// Vector of `T` (Stef's `non_zero<vec<T>>`) that is guaranteed to not be empty.
pub type NonZeroVec<T> = NonZero<Vec<T>>;
/// Hash map (Stef's `non_zero<hash_map<K ,V>>`) that is guaranteed to not be empty.
pub type NonZeroHashMap<K, V> = NonZero<HashMap<K, V>>;
/// Hash set (Stef's `non_zero<hash_set<T>>`) that is guaranteed to not be empty.
